...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Permissions: Accessibility (hotkey + paste) and Microphone.
        Permissions.ensureAccessibility(prompt: true)
        Task { _ = await Permissions.ensureMicrophone() }

        // Global right-Command hotkey + dictation state machine.
        dictation.start()

        // Load the Parakeet model into memory (downloads on first run).
